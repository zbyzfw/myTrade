...
def start_trade(行情, ticks):  # 开仓判断
    ...
    if status == '等待开仓':
        ...
        if (oldt > newt) and (newt not in 挂空单子列表.values()):  # 价格下跌,做空
            if 空仓之和 >= 多仓之和:  # 做空时空仓多,只做一手
                status = '等待开仓成交'
                ...
                开单数量 = 订单数量差
                上一个价格 = newt
        elif (oldt > newt) and (newt in 挂空单子列表.values()):  # 价格下跌,做空
        ...
        elif (oldt < newt) and (行情.ask_price1 not in 挂多单子列表.values()):  # 价格上涨,做多
            if 多仓之和 >= 空仓之和:  # 做多时多仓多,只做一手
                ...
                开单数量 = 订单数量差
                上一个价格 = newt
        elif (oldt - newt < 0) and (行情.ask_price1 in 挂空单子列表.values()):  # 价格上涨且已有空单记录,做多
...
def 委托挂单(行情, ticks):
    ...
    if status == "等待开仓成交":
        ...
        if a.is_dead:
            print('开单成交---')
            if 开单方向1 == "SELL" and a.offset == 'OPEN':
                挂单数量 = 开单数量
                print(a.trade_price)
                ...
                # if tick =
                status = '检测挂价订单委托'
            elif 开单方向1 == 'BUY' and a.offset == 'OPEN':
                挂单数量 = 开单数量
                # 当前价格 = ticks["ask_price1"].tolist()[-1] - 一跳价格#BUY
...
def 检测挂价订单(行情, ticks):
    ...
    if status == "检测挂价订单委托":
        ...
        # else:
        #     当前价格 = ticks["ask_price1"].tolist()[-1]#BUY
        # 下单时间 = a['insert_date_time']
        # if 未成交手数1 != 0:
        #     当前状态 = "等待平仓"#执行平仓检测
        #     print("执行平仓检测")
        #     return
        # if BUYs(委托价格1 - 当前价格) > 委托远离几跳撤单 * 一跳价格:
...
def 平所有(symbol):  # 直接用
    ...
    if 多仓1 != 0:
        api.insert_order(symbol=symbol, direction="BUY", offset="CLOSE", volume=多仓1)
    if 多仓2 != 0:
        if symbol[:4] == "SHFE":
            api.insert_order(symbol=symbol, direction="BUY", offset="CLOSETODAY", volume=多仓2)
        else:
            api.insert_order(symbol=symbol, direction="BUY", offset="CLOSE", volume=多仓2)
    if 空仓1 != 0:
        api.insert_order(symbol=symbol, direction="SELL", offset="CLOSE", volume=空仓1)
    if 空仓2 != 0:
        if symbol[:4] == "SHFE":
            api.insert_order(symbol=symbol, direction="SELL", offset="CLOSETODAY", volume=空仓2)
        else:
            api.insert_order(symbol=symbol, direction="SELL", offset="CLOSE", volume=空仓2)
